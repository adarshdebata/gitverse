...
function StepControls({ step, maxStep, onPrev, onNext, onReset, label }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
      ...
      <button className="btn" onClick={onPrev} disabled={step === 0}>← Prev</button>
      <button className="btn btn-primary" onClick={onNext} disabled={step === maxStep}>Next →</button>
      {label && (
...
export function StagingViz() {
  ...
  const Zone = ({ label, icon, color, files, placeholder }) => (
    ...
    >
      <div style={{
        fontSize: 11, fontWeight: 700, color: files.length > 0 ? color : 'var(--muted)',
        marginBottom: 10, fontFamily: 'IBM Plex Mono', textTransform: 'uppercase', letterSpacing: '0.06em',
        transition: 'color 0.4s ease',
      }}>
        {icon} {label}
      </div>
      {files.length === 0
        ? <div style={{ color: 'var(--dim)', fontSize: 11, fontFamily: 'IBM Plex Mono' }}>{placeholder}</div>
        : files.map((f, i) => (
            <div key={i} className="animate-slide-r" style={{
              display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5,
              animationDelay: `${i * 60}ms`,
            }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, flexShrink: 0 }} />
              <span style={{ fontSize: 11, fontFamily: 'IBM Plex Mono', color: 'var(--text)' }}>{f}</span>
            </div>
          ))}
    </div>
  )
  ...
  return (
    <div>
      <StepControls
        step={step} maxStep={STAGING_STEPS.length - 1}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr auto 1fr', gap: 8, alignItems: 'center' }}>
        <Zone label="Working Directory" icon="📁" color="var(--cyan)"
          files={s.working} placeholder="(clean)" />
        <div style={{ textAlign: 'center', fontSize: 10, color: step >= 1 ? 'var(--accent)' : 'var(--dim)',
          fontFamily: 'IBM Plex Mono', transition: 'color 0.4s', lineHeight: 1.6, padding: '0 4px' }}>
          git add<br/>→
        </div>
        <Zone label="Staging Area (Index)" icon="📋" color="var(--accent)"
          files={s.staged} placeholder="(empty)" />
        <div style={{ textAlign: 'center', fontSize: 10, color: step >= 3 ? 'var(--emerald)' : 'var(--dim)',
          fontFamily: 'IBM Plex Mono', transition: 'color 0.4s', lineHeight: 1.6, padding: '0 4px' }}>
          git commit<br/>→
        </div>
        <Zone label="Repository (.git)" icon="🗄️" color="var(--emerald)"
          files={s.history} placeholder="(no commits)" />
      </div>
...
const REBASE_STEPS = [
  ...
  },
  {
    ...
    scenario: 'rebase_before',
    callout: { type: 'warn', text: 'Commits D and E are saved as patches. Their SHAs will change.' },
  },
  {
    ...
    callout: { type: 'info', text: "D' and E' have new SHAs. Same code changes. Different hashes." },
  },
  {
    ...
    scenario: 'rebase_after',
    callout: { type: 'warn', text: '⚠️ SHAs changed — git push --force-with-lease required if already pushed.' },
  },
]
...
export function RebaseViz() {
  ...
  return (
    <div>
      <StepControls
        step={step} maxStep={REBASE_STEPS.length - 1}
        onPrev={() => setStep(s => s - 1)}
        onNext={() => setStep(s => s + 1)}
        onReset={() => setStep(0)}
      ...
      <div className="gitverse-card" style={{ padding: 16, marginBottom: 14 }}>
        <div className="font-heading" style={{ fontWeight: 700, fontSize: 14, marginBottom: 8, color: 'var(--accent)' }}>
          {s.title}
        </div>
        <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.7, marginBottom: s.callout ? 12 : 0 }}>
          {s.desc}
        </p>
        {s.callout && (
          <Alert type={s.callout.type}>{s.callout.text}</Alert>
        )}
      </div>
...
const RESET_MODE_DATA = {
  '--soft': {
    ...
    index:   { label: 'Index (Staging)', affected: false, result: 'Unchanged — changes still staged' },
...
export function ResetViz() {
      ...
      <div
        ...
        style={{
          ...
          borderColor: layer.affected
            ? (data.danger === 'high' && layer.label.includes('Working') ? 'var(--rose)' : 'var(--amber)')
            : 'var(--border)',
          background: layer.affected
            ? (data.danger === 'high' && layer.label.includes('Working') ? 'var(--rose-dim)' : 'var(--amber-dim)')
            : 'var(--card)',
          transition: 'all 0.3s ease',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Badge variant={layer.affected ? (data.danger === 'high' && layer.label.includes('Working') ? 'high' : 'medium') : 'safe'}>
  ...
  return (
    <div>
      ...
      <Alert type={data.danger === 'high' ? 'danger' : data.danger === 'safe' ? 'success' : 'warn'}
        className="mb-3" style={{ marginBottom: 14 }}>
        <span>{data.desc}</span>
      </Alert>
      ...
      <div style={{ marginBottom: 12, padding: 12, background: 'var(--surface)', borderRadius: 8,
        border: '1px solid var(--border)', fontSize: 12, color: 'var(--muted)', lineHeight: 1.6 }}>
      ...
      {mode === '--hard' && (
        <div className="animate-fade-in" style={{ marginTop: 12 }}>
          <Alert type="info">
            ...
            <code>git reflog</code> — commits survive for 90 days.
            Then: <code>git reset --hard HEAD@{'{n}'}</code>
          </Alert>
        </div>
      )}
    </div>
...
const STASH_STEPS = [
  ...
  },
  {
    ...
    stash:   [],
    working: ['src/api.js (auth refactor restored)', 'src/auth.js (WIP restored)', 'hotfix/login.js (applied too)'],
    branch:  'feat/auth',
  },
]
...
export function StashViz() {
  ...
  return (
    <div>
      <StepControls
        step={step} maxStep={STASH_STEPS.length - 1}
        onPrev={() => setStep(s => s - 1)}
        onNext={() => setStep(s => s + 1)}
        onReset={() => setStep(0)}
      ...
      <div style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 14, lineHeight: 1.65 }}>{s.desc}</div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div className="gitverse-card" style={{ padding: 14, borderColor: 'var(--amber)', minHeight: 110 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--amber)', marginBottom: 10,
            fontFamily: 'IBM Plex Mono', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            🗃️ Stash Stack
          </div>
          {s.stash.length === 0
            ? <div style={{ color: 'var(--dim)', fontSize: 11, fontFamily: 'IBM Plex Mono' }}>(empty)</div>
            : s.stash.map((entry, i) => (
                <div key={i} className="animate-slide-r" style={{
                  fontSize: 11, fontFamily: 'IBM Plex Mono', padding: '5px 8px',
                  background: 'var(--amber-dim)', borderRadius: 5, marginBottom: 5, color: 'var(--text)',
                }}>
                  {entry}
                </div>
              ))}
        ...
        <div className="gitverse-card" style={{ padding: 14, borderColor: 'var(--cyan)', minHeight: 110 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--cyan)', marginBottom: 10,
            fontFamily: 'IBM Plex Mono', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            📁 Working Tree ({s.branch})
          </div>
          {s.working.length === 0
            ? <div style={{ color: 'var(--emerald)', fontSize: 11, fontFamily: 'IBM Plex Mono' }}>✓ clean</div>
            : s.working.map((f, i) => (
                <div key={i} style={{ fontSize: 11, fontFamily: 'IBM Plex Mono', color: 'var(--text)',
...
const REFLOG_ENTRIES = [
  { sha: 'd4e5f6g', idx: 0, action: 'commit',    msg: 'Fix session expiry bug',               ago: '2h',  highlight: true,   recovery: false },
  { sha: 'c3d4e5f', idx: 1, action: 'commit',    msg: 'Implement JWT refresh tokens',         ago: '5h',  highlight: false,  recovery: false },
  { sha: 'b2c3d4e', idx: 2, action: 'checkout',  msg: 'moving from feature/auth to main',     ago: '7h',  highlight: false,  recovery: false },
  { sha: 'b8c9d0e', idx: 3, action: 'commit',    msg: 'Auth middleware [BRANCH DELETED]',     ago: '8h',  highlight: false,  recovery: true  },
  { sha: 'b2c3d4e', idx: 4, action: 'commit',    msg: 'Add user authentication',              ago: '1d',  highlight: false,  recovery: false },
  { sha: 'a1b2c3d', idx: 5, action: 'commit',    msg: 'Initial commit',                       ago: '3d',  highlight: false,  recovery: false },
]
          ...
          <tbody>
            {REFLOG_ENTRIES.map(e => (
              ...
              >
                ...
                <td>
                  <code style={{ color: 'var(--muted)', fontSize: 11 }}>HEAD@{'{' + e.idx + '}'}</code>
                </td>
                <td>
                  <span style={{ fontSize: 11, color: e.action === 'commit' ? 'var(--emerald)' : 'var(--amber)',
                ...
                <td style={{ fontSize: 12 }}>
                  {e.msg}
                  {e.recovery && (
                    <Badge variant="high" className="ml-2" style={{ marginLeft: 8 }}>recoverable</Badge>
                  )}
                </td>
...
export function MergeConflictViz() {
  ...
  return (
    <div>
      <StepControls
        step={step} maxStep={CONFLICT_STEPS.length - 1}
        onPrev={() => setStep(s => s - 1)}
        onNext={() => setStep(s => s + 1)}
        onReset={() => setStep(0)}
      ...
      <div className="gitverse-card" style={{ padding: 14, marginBottom: 14, borderLeft: '3px solid var(--accent)' }}>
        <div className="font-heading" style={{ fontWeight: 700, marginBottom: 6, color: 'var(--accent)' }}>
          {s.title}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="gitverse-card" style={{ padding: 12, borderColor: 'var(--accent)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', marginBottom: 8,
              fontFamily: 'IBM Plex Mono' }}>main — Dev A's version</div>
            <div className="codeblock" style={{ fontSize: 12 }}>{s.devA}</div>
          </div>
          <div className="gitverse-card" style={{ padding: 12, borderColor: 'var(--cyan)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--cyan)', marginBottom: 8,
              fontFamily: 'IBM Plex Mono' }}>feature/auth — Dev B's version</div>
            <div className="codeblock" style={{ fontSize: 12 }}>{s.devB}</div>
          </div>
        </div>
      )}
      ...
      {s.phase === 'conflict' && (
        <div>
          <Alert type="danger" style={{ marginBottom: 12 }}>
            Git writes conflict markers into the file. The file is now unmerged — you must resolve manually.
          </Alert>
          <div className="codeblock">
            <pre style={{ margin: 0 }}>
              <span style={{ color: 'var(--rose)' }}>{'<<<<<<< HEAD (main)'}</span>{'\n'}
              <span style={{ color: 'var(--accent)' }}>{'const secret = process.env.JWT_SECRET'}</span>{'\n'}
              <span style={{ color: 'var(--muted)' }}>{'======='}</span>{'\n'}
              <span style={{ color: 'var(--cyan)' }}>{'const secret = require("./config").jwtSecret'}</span>{'\n'}
...
const BISECT_STEPS = [
  ...
  { midpoint: 5,    tested: [4, 5, 7], marked: { good: [0, 4], bad: [5, 7, 9] }, step: 'found', culprit: 5 },
]
...
export function BisectViz() {
  ...
  return (
    <div>
      <StepControls
        step={step} maxStep={BISECT_STEPS.length - 1}
        onPrev={() => setStep(s => s - 1)}
        onNext={() => setStep(s => s + 1)}
        onReset={() => setStep(0)}
      ...
      <div style={{ color: 'var(--muted)', fontSize: 13, marginBottom: 16 }}>
        ...
        <strong style={{ color: 'var(--accent)' }}>4 steps</strong> to find the bug.
        Math: log₂(10) ≈ 3.3 → 4 steps.
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
        {BISECT_COMMITS.map((c, i) => {
          ...
          if (isGoodMarked) { bg = 'var(--emerald-dim)'; border = 'var(--emerald)'; color = 'var(--emerald)' }
          if (isBadMarked)  { bg = 'var(--rose-dim)';    border = 'var(--rose)';    color = 'var(--rose)' }
          if (isMidpoint)   { bg = 'var(--accent-dim)';  border = 'var(--accent)';  color = 'var(--accent)' }
          if (isCulprit)    { bg = 'var(--rose-dim)';    border = 'var(--rose)';    color = 'var(--rose)'; }

          return (
            <div key={i} style={{
              width: 52, padding: '8px 4px', borderRadius: 7, border: `1.5px solid ${border}`,
              background: bg, textAlign: 'center', transition: 'all 0.3s ease', fontSize: 10,
              fontFamily: 'IBM Plex Mono',
            }}>
              <div style={{ color, fontWeight: 700 }}>c{c.n}</div>
              {c.label && <div style={{ color: 'var(--muted)', fontSize: 8, marginTop: 2 }}>{c.label}</div>}
              {isGoodMarked && <div style={{ color: 'var(--emerald)', fontSize: 8 }}>✓ good</div>}
              {isBadMarked  && <div style={{ color: 'var(--rose)',    fontSize: 8 }}>✗ bad</div>}
              {isMidpoint   && !isBadMarked && !isCulprit && <div style={{ color: 'var(--accent)', fontSize: 8 }}>← test</div>}
      ...
      {s.step === 'found' ? (
        <Alert type="success">
          <strong>Found!</strong> Commit c{s.culprit} is the first bad commit.
          Run <code>git show c{s.culprit}</code> to inspect it.
          Then <code>git bisect reset</code> to return to HEAD.
        </Alert>
      ) : (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          {s.midpoint && (
            <div className="gitverse-card" style={{ padding: 12, flex: 1, borderColor: 'var(--accent)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--accent)', marginBottom: 6,
                fontFamily: 'IBM Plex Mono' }}>Testing c{s.midpoint}</div>
...
Release v2.0.0 - JWT Authentication`,
  ...
  return (
    <div>
        ...
        {objects.map(obj => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: obj.color, fontFamily: 'IBM Plex Mono',
                textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                {obj.type}
              </span>
              <code style={{ fontSize: 9, color: 'var(--amber)', marginLeft: 'auto' }}>{obj.sha}...</code>
            </div>
      ...
      {selected_obj && (
        <div className="animate-fade-in gitverse-card" style={{ padding: 16, borderColor: selected_obj.color }}>
          <div className="font-heading" style={{ fontWeight: 700, fontSize: 15, marginBottom: 12,
            color: selected_obj.color }}>
            {selected_obj.icon} {selected_obj.title}
          </div>
          <CodeBlock code={selected_obj.content} style={{ marginBottom: 12 }} />
          <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.75 }}>{selected_obj.desc}</p>
        </div>
      )}
      ...
      <div style={{ marginTop: 20 }}>
        ...
        <svg viewBox="0 0 640 150" style={{ width: '100%', maxHeight: 150 }}>
          {/* HEAD → commit */}
          <text x={20} y={75} fontSize={10} fill="var(--accent)" fontFamily="IBM Plex Mono" fontWeight="700">HEAD → main →</text>

          {/* commit box */}
          <rect x={140} y={40} width={110} height={70} rx={6} fill="var(--card)" stroke="var(--accent)" strokeWidth={1.5} />
          <text x={195} y={58} textAnchor="middle" fontSize={9} fill="var(--accent)" fontFamily="IBM Plex Mono" fontWeight="700">COMMIT</text>
          <text x={195} y={73} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">tree: b2c3d4</text>
          <text x={195} y={86} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">parent: z9y8x7</text>
          <text x={195} y={99} textAnchor="middle" fontSize={8} fill="var(--accent)" fontFamily="IBM Plex Mono">c3d4e5f</text>

          {/* tree box */}
          <rect x={310} y={25} width={110} height={70} rx={6} fill="var(--card)" stroke="var(--emerald)" strokeWidth={1.5} />
          <text x={365} y={43} textAnchor="middle" fontSize={9} fill="var(--emerald)" fontFamily="IBM Plex Mono" fontWeight="700">TREE</text>
          <text x={365} y={58} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">api.js → a1b2c3</text>
          <text x={365} y={71} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">auth.js → e5f6g7</text>
          <text x={365} y={84} textAnchor="middle" fontSize={8} fill="var(--emerald)" fontFamily="IBM Plex Mono">b2c3d4</text>

          {/* blob boxes */}
          <rect x={490} y={10} width={90} height={40} rx={6} fill="var(--card)" stroke="var(--cyan)" strokeWidth={1.5} />
          <text x={535} y={28} textAnchor="middle" fontSize={9} fill="var(--cyan)" fontFamily="IBM Plex Mono" fontWeight="700">BLOB</text>
          <text x={535} y={42} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">api.js content</text>

          <rect x={490} y={80} width={90} height={40} rx={6} fill="var(--card)" stroke="var(--cyan)" strokeWidth={1.5} />
          <text x={535} y={98} textAnchor="middle" fontSize={9} fill="var(--cyan)" fontFamily="IBM Plex Mono" fontWeight="700">BLOB</text>
          <text x={535} y={112} textAnchor="middle" fontSize={8} fill="var(--muted)" fontFamily="IBM Plex Mono">auth.js content</text>

          {/* Arrows */}
          <line x1={250} y1={75} x2={310} y2={60} stroke="var(--accent)" strokeWidth={1.5} markerEnd="url(#a1)" />
          <line x1={420} y1={50} x2={490} y2={30} stroke="var(--emerald)" strokeWidth={1.5} markerEnd="url(#a1)" />
          <line x1={420} y1={60} x2={490} y2={100} stroke="var(--emerald)" strokeWidth={1.5} markerEnd="url(#a1)" />
