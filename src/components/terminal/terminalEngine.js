...
${t.dim('Other:')}
  ...
  if (sub === 'add') {
    ...
    if (target === '-p' || target === '--patch') {
      return {
        ...
        repoUpdate: { staged: [...repo.staged, 'src/api.js (partial)'], unstaged: repo.unstaged.filter(f => f !== 'src/api.js') },
      }
    }
    ...
    if (target === '--dry-run') {
      const all = [...repo.unstaged, ...(repo.untracked || [])]
      return {
        output: all.map(f => `${t.success('add')} '${f}'`).join('\n') || t.dim('nothing would be staged'),
        repoUpdate: null,
      }
    }
  ...
  if (sub === 'commit') {
    ...
    if (amend) {
      if (repo.commits.length === 0) return { output: t.error('nothing to amend'), repoUpdate: null }
      ...
      return {
        ...
        repoUpdate: {
          commits: [
            ...
            { h: newHash, msg: newMsg, time: 'just now' },
          ],
          staged: [],
          head: newHash,
        },
  ...
  if (sub === 'branch') {
    ...
    if (deleteFl && name) {
      if (name === repo.branch) {
        return { output: t.error(`error: cannot delete the currently checked-out branch '${esc(name)}'`), repoUpdate: null }
      }
    ...
    if (name) {
      if (repo.branches.includes(name)) {
        return { output: t.error(`fatal: A branch named '${esc(name)}' already exists.`), repoUpdate: null }
      }
      return {
  ...
  if (sub === 'checkout') {
    ...
    return {
      output: [
        ...
        t.warn(`   Use ${t.success('git switch')} for branches, ${t.success('git restore')} for files.`),
        '',
        name && createFl
  ...
  if (sub === 'stash') {
    ...
    if (!stashSub || stashSub === 'push') {
      ...
      return {
        ...
        repoUpdate: {
          stash: [
            { files: [...repo.staged, ...repo.unstaged], msg: desc },
            ...repo.stash,
          ],
          staged: [], unstaged: [],
        },
    ...
    if (stashSub === 'list') {
      return {
        output: repo.stash.length === 0
          ? t.dim('(no stash entries)')
          : repo.stash.map((s, i) =>
              `${t.hash('stash@{' + i + '}')}${t.dim(': ')}: ${esc(s.msg)}`
            ).join('\n'),
        repoUpdate: null,
      }
    }
    ...
    if (stashSub === 'drop') {
      if (repo.stash.length === 0) return { output: t.error('error: No stash entry'), repoUpdate: null }
      return {
  ...
  if (sub === 'cherry-pick') {
    ...
    return {
      ...
      repoUpdate: sha ? {
        commits: [
          ...repo.commits,
          { h: hash, msg: `Cherry-picked: ${sha}`, time: 'just now' },
        ],
        head: hash,
      } : null,
    }
      ...
      return {
        output: [
          ...
          ...recentCommits.map((c, i) =>
            `${t.success('pick')} ${t.hash(c.h)} ${esc(c.msg)}`
          ),
          '',
          t.dim('# Commands:'),
  ...
  if (sub === 'bisect') {
    ...
    if (bisectSub === 'start') return { output: t.success('Bisect started. Mark commits: git bisect bad / git bisect good <sha>'), repoUpdate: null }
    if (bisectSub === 'bad')   return { output: t.info('Marked as bad. Bisecting: ~8 steps remaining.'), repoUpdate: null }
    if (bisectSub === 'good')  return { output: t.success('Marked as good. Bisecting: ~4 steps remaining.'), repoUpdate: null }
    if (bisectSub === 'reset') return { output: t.success('Bisect reset. Back to HEAD.'), repoUpdate: null }
    if (bisectSub === 'run')   return {
      output: [
    ...
    return { output: t.info('git bisect start → git bisect bad → git bisect good <sha>'), repoUpdate: null }
  }
