...
create a partial blob object — this is why staged and working-tree versions of a file can differ.`,
    ...
    enterprise_note:
      "Use pre-commit hooks (Husky + lint-staged) to run linting/formatting on staged files. Integrate git-secrets or detect-secrets to prevent credential staging.",
  },
...
content — change anything and you get a different commit.`,
    syntax: [
      ...
      { cmd: "git commit --squash <hash>", desc: "Create squash! commit for interactive rebase" },
      {
        ...
        desc: "Create commit with no file changes (CI trigger)",
      },
    ],
    examples: [
      {
...
git rebase -i --autosquash HEAD~5`,
    ...
    safer_alternatives: [
      {
        cmd: "git commit --fixup + rebase -i --autosquash",
        reason: "Cleaner than amending old commits",
      },
    ],
...
different times produce different commit SHAs. Git guarantees integrity this way.`,
    ...
    enterprise_note:
      "Enforce Conventional Commits via commitlint + Husky. This enables automated CHANGELOG generation (standard-version / semantic-release) and automatic semver bumping.",
  },
    ...
    syntax: [
      {
        ...
        desc: "⚠️ Discard working tree changes (irreversible without stash)",
      },
      { cmd: "git restore --staged <file>", desc: "Unstage file — keep working tree changes" },
      {
        cmd: "git restore --staged --worktree <file>",
        desc: "Unstage AND discard working tree changes",
      },
      { cmd: "git restore --source=<commit> <file>", desc: "Restore file from a specific commit" },
      {
        cmd: "git restore --source=HEAD~3 src/",
        desc: "Restore entire directory from 3 commits ago",
      },
...
are overwritten and not in reflog.`,
  ...
  checkout: {
    ...
    deprecated: true,
    deprecation_note:
      "git checkout is overloaded — it does both branch switching AND file restoration. Git 2.23+ introduces git switch (branches) and git restore (files) as explicit replacements. checkout still works but its dual behavior confuses beginners.",
...
checkout <branch> all do very different things). Still fully functional but superseded.`,
    syntax: [
      ...
      { cmd: "git checkout -- <file>", desc: "⚠️ Use: git restore <file>", deprecated: true },
      {
        ...
        desc: "⚠️ Use: git restore --source=<commit> <file>",
        deprecated: true,
      },
    ],
    modern_alternatives: [
...
have based their work on.`,
    syntax: [
      ...
      {
        cmd: 'git rebase --exec "npm test" HEAD~5',
        desc: "Run command after each replayed commit",
      },
    ],
    examples: [
      {
...
git rebase --onto main feature-a your-feature`,
    ...
    recovery:
      "If you rebased wrongly: git reflog to find original commit SHA → git reset --hard <sha> to restore it. Your original commits survived in the reflog for 90 days.",
...
processes each entry sequentially, stopping for conflicts or "edit" instructions.`,
    ...
    enterprise_note:
      "Configure branch protection to require linear history (enforces rebase merges). Use --force-with-lease which checks remote ref before overwriting — prevents accidentally overwriting a colleague's push.",
  },
...
The "three trees" mental model is essential to understand before using this command.`,
    syntax: [
      ...
      { cmd: "git reset <file>", desc: "Unstage specific file (--mixed mode, file only)" },
      {
        ...
        desc: "⚠️ Nuclear: sync to remote, discarding everything local",
      },
      {
        cmd: "git reset ORIG_HEAD",
        desc: "Undo last merge/rebase (Git sets ORIG_HEAD before these)",
      },
    ],
    modes: [
      {
        ...
        index: "unchanged",
        working: "unchanged",
        useCase: "Undo last commit, keep changes staged for re-commit",
      },
      {
        flag: "--mixed",
        head: "✓ moved",
        index: "✓ reset",
        working: "unchanged",
        useCase: "Undo last commit, unstage everything (default)",
      },
      {
        flag: "--hard",
        head: "✓ moved",
        index: "✓ reset",
        working: "✓ reset",
        useCase: "⚠️ Full destroy — only safe if changes are truly unwanted",
      },
    ],
    examples: [
      {
...
git log origin/main..HEAD  # YOUR commits that would be lost`,
    ...
    recovery:
      "If you reset --hard and lost commits: git reflog → find the SHA you were at → git reset --hard <sha> OR git checkout -b recovery-branch <sha>. Commits survive in reflog for 90 days.",
...
still there, just unreachable via refs — until git gc runs (usually after 90 days).`,
    visualization: "reset",
    enterprise_note:
      "Enable branch protection on main/develop to prevent force pushes. require pull request reviews prevents accidental reset + force push disasters.",
  },
...
This is the enterprise-safe, audit-compliant way to undo changes on public/shared branches.`,
    syntax: [
      ...
      {
        cmd: "git revert -m 1 <merge-sha>",
        desc: "Revert a merge commit (-m 1 = keep parent 1 / mainline)",
      },
...
reflog lets you recover. Entries persist for 90 days by default. This is your time machine.`,
    syntax: [
      ...
      { cmd: "git reflog show HEAD", desc: "Same as git reflog (HEAD is default)" },
      {
        ...
        desc: "Manually expire entries older than 30 days",
      },
...
between tasks without creating WIP commits. Push onto the stack, pop off when you return.`,
    syntax: [
      ...
      {
        cmd: 'git stash push -m "description"',
        desc: "Stash with a meaningful description (recommended)",
      },
      ...
      { cmd: "git stash clear", desc: "⚠️ Delete ALL stashes permanently" },
      {
        ...
        desc: "Create new branch from stash (avoids pop conflicts)",
      },
    ],
    examples: [
      {
...
or selectively applying specific commits across branches.`,
    syntax: [
      ...
      {
        cmd: "git cherry-pick --no-commit <sha>",
        desc: "Apply changes without committing (stage only)",
      },
...
three-way merge (with merge commit) depending on branch divergence.`,
    syntax: [
      { cmd: "git merge <branch>", desc: "Merge branch into current branch" },
      {
        ...
        desc: "Force a merge commit even if fast-forward is possible",
      },
      {
        cmd: "git merge --squash <branch>",
        desc: "Squash all branch commits into one staged commit",
      },
      ...
      {
        cmd: "git merge -X ours <branch>",
        desc: "Auto-resolve conflicts using current branch version",
      },
      {
        cmd: "git merge -X theirs <branch>",
        desc: "Auto-resolve conflicts using merging branch version",
      },
...
what it does. Note: "blame" is about understanding context, not assigning fault.`,
    ...
    internals:
      "Reads each line of the file and walks backwards through commit history finding the most recent commit that changed each line. -M and -C options use diff similarity detection to track code movement.",
    visualization: "commit",
  },
...
--force-with-lease instead of --force to prevent overwriting teammates' work.`,
    ...
    internals:
      'Sends pack data (missing objects) to the remote, then requests the remote update its refs. Force push bypasses the "fast-forward only" check. --force-with-lease includes the expected SHA of the remote ref — if it differs, the push is rejected.',
    visualization: "commit",
  },
...
working tree or current branch. The safe way to see what the remote has before integrating.`,
    ...
    internals:
      "Downloads missing commit/tree/blob objects from the remote and updates remote-tracking refs in .git/refs/remotes/. Does NOT modify HEAD, local branches, or working tree.",
    visualization: "commit",
  },
...
changes before committing or understanding what diverged between branches.`,
    ...
    internals:
      "Reads blob objects and applies the Myers diff algorithm to produce unified diff output. --staged reads from .git/index vs HEAD commit tree. Three-dot notation (A...B) finds the merge base first.",
    visualization: "staging",
  },
};
...
export const COMMAND_CATEGORIES = {
  daily: {
    ...
    color: "var(--emerald)",
    desc: "Commands used in every development session",
  },
  advanced: {
    label: "Advanced",
    color: "var(--amber)",
    desc: "History manipulation, deep debugging, complex workflows",
  },
  recovery: {
    label: "Recovery",
    color: "var(--rose)",
    desc: "Getting out of trouble and recovering lost work",
  },
};
