  ...
  return (
    <div className="animate-fade-up">
      ...
      <Collapsible icon={<ScrollText size={18} />} title="What is Version Control?" defaultOpen>
        ...
        <p>
          Git's radical innovation: every clone is a full backup. "origin" is just a convention.
        </p>
      </Collapsible>
      ...
      <Collapsible icon={<Globe size={18} />} title="Distributed vs Centralized VCS">
        ...
        <CodeBlock
          code={`# Git thinks about remotes as just named URLs
...
git fetch upstream && git rebase upstream/main`}
      ...
      <Collapsible icon={<RefreshCw size={18} />} title="The Three States of Git">
          ...
          {[
            {
              state: "Modified",
              color: "var(--amber)",
              desc: "Changed in working directory but NOT staged. Git knows about the file but hasn't prepared it for commit.",
            },
            {
              state: "Staged",
              color: "var(--accent)",
              desc: "Marked in the Index (.git/index) to go into the NEXT commit. Exact snapshot of what will be committed.",
            },
            {
              state: "Committed",
              color: "var(--emerald)",
              desc: "Safely stored in the local Git database as an immutable object with a SHA-1 hash. Permanent record.",
            },
          ].map((s) => (
            <div
              key={s.state}
              className="gitverse-card"
              style={{ padding: 14, borderLeft: `3px solid ${s.color}` }}
            >
        ...
        <p style={{ color: "var(--muted)", fontSize: 13 }}>
          This maps to three physical areas: Working Tree (disk files), Index (.git/index),
      ...
      <Collapsible icon={<BookMarked size={18} />} title="Understanding HEAD — The Key Pointer">
        ...
        <CodeBlock
          code={`# Attached HEAD (normal state)
$ cat .git/HEAD
...
# Save with: git switch -c new-branch`}
          style={{ marginBottom: 12 }}
        />
        <Alert type="warn">
          <strong>Detached HEAD:</strong> Any commits you make won't be on any branch. They'll
          appear in reflog for 90 days, then be garbage collected. Always run{" "}
          <code>git switch -c branch-name</code> to save your work.
        </Alert>
      </Collapsible>
      ...
      {tab === "objects" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 20 }}>
            Every Git object is stored in <code>.git/objects/</code> by its SHA-1 hash. Four types:{" "}
            <strong>blob</strong> (file content), <strong>tree</strong> (directory),
          ...
          <div style={{ marginTop: 20 }}>
            <InfoCard icon={<KeyRound size={18} />} title="SHA-1 Content Addressing" accentColor="var(--amber)">
              ...
              <p style={{ marginTop: 8 }}>
                Two identical files = same SHA = same blob object = stored ONCE. Change one byte
                anywhere in history and every downstream SHA changes. This is what makes Git
                tamper-evident: you cannot secretly modify history.
              </p>
            </InfoCard>
          </div>
        </div>
      ...
      {tab === "refs" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 20 }}>
            ...
            <code>.git/refs/</code> as plain text files containing a 40-char SHA-1. A branch IS just
            a file containing a SHA. That's all.
          </Alert>
          <CodeBlock
            code={`# A branch is just a 41-byte file
            ...
            {[
              {
                ...
                desc: "A branch = movable pointer to a commit. Creating a branch = writing one 41-byte file. Free and instant.",
              },
              {
                type: "Remote Refs",
                path: ".git/refs/remotes/origin/main",
                desc: "Read-only cached copies of remote branch tips. Updated on git fetch. Never commit directly to these.",
              },
              {
                type: "Tags",
                path: ".git/refs/tags/v2.0.0",
                desc: "Lightweight: SHA of a commit. Annotated: SHA of a tag object (with message + tagger). Always use annotated for releases.",
              },
              {
                type: "ORIG_HEAD",
                path: ".git/ORIG_HEAD",
                desc: "Saved by merge, rebase, cherry-pick. Points to pre-operation HEAD. Allows: git reset --hard ORIG_HEAD to undo.",
              },
            ].map((r) => (
              <div key={r.type} className="gitverse-card" style={{ padding: 14 }}>
                <div
                  style={{ fontWeight: 700, fontSize: 13, color: "var(--accent)", marginBottom: 6 }}
                >
                  {r.type}
                </div>
                <code
      ...
      {tab === "index" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 16 }}>
            The Index (staging area) is a binary file at <code>.git/index</code>. It's a sorted list
            of file paths mapped to blob SHAs, representing the <em>next commit's tree</em> before
            it's committed.
          </Alert>
          <CodeBlock
            code={`# Inspect the index directly
git ls-files --stage
...
# 3 = theirs (during merge conflict)`}
          ...
          >
            <code>git add -p</code> works by reading the index, computing diff against HEAD, and
            letting you choose which diff hunks to write as new partial blob objects. The index can
            have a DIFFERENT version of a file than both HEAD and working tree. This three-way
            divergence is exactly what makes git add -p possible.
          </InfoCard>
        </div>
      )}
      ...
      {tab === "packfiles" && (
        <div className="animate-fade-in">
          <Alert type="warn" style={{ marginBottom: 16 }}>
            Without packfiles, Git stores every version of every file as a separate blob. For a
            1000-commit repo with a frequently-changed file, that's 1000 blobs. Packfiles compress
            them with delta encoding — storing only differences.
          </Alert>
          <CodeBlock
            code={`# Manually trigger garbage collection + repacking
git gc
# or more aggressive:
...
git gc --dry-run`}
      ...
      {tab === "hooks" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 16 }}>
            Git hooks are scripts in <code>.git/hooks/</code> that fire at lifecycle events. They're
            NOT committed to the repo — use <strong>Husky</strong> or <strong>lefthook</strong> to
            ...
            <table className="comp-table">
              ...
              <tbody>
                {[
                  [
                    "pre-commit",
                    "Before commit created",
                    "Abort commit",
                    "Linting, tests, secret scanning",
                  ],
                  [
                    "commit-msg",
                  ...
                  [
                    ...
                    "Ignored",
                    "Notifications, local CI trigger",
                  ],
                  [
                    "pre-push",
                  ...
                  [
                    ...
                    "Abort rebase",
                    "Safety checks on shared branches",
                  ],
                  ...
                  [
                    "post-checkout",
                    "After checkout/switch",
                    "Ignored",
                    "Update submodules, node_modules",
                  ],
                ].map(([h, f, e, u]) => (
                  <tr key={h}>
                    <td>
                      <code style={{ color: "var(--cyan)" }}>{h}</code>
                    </td>
          ...
          <CodeBlock
            code={`# Install Husky for team-wide hooks
  ...
  return (
    <div className="animate-fade-up">
      ...
      {tab === "gitflow" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 20 }}>
            GitFlow works best for <strong>scheduled-release software</strong> (mobile apps, desktop
            software, versioned APIs). For SaaS with continuous delivery, Trunk-Based Development is
            usually better.
          </Alert>
          <div
              ...
              {[
                {
                  b: "main",
                  c: "var(--accent)",
                  desc: "Production-only. Every commit is a tagged release.",
                },
                {
                  b: "develop",
                  c: "var(--emerald)",
                  desc: "Integration branch. All features merge here first.",
                },
                {
                  b: "feature/*",
                  c: "var(--cyan)",
                  desc: "New features. Branch from develop, merge to develop.",
                },
                {
                  b: "release/*",
                  c: "var(--amber)",
                  desc: "Release prep. Bug fixes only. Merges to main + develop.",
                },
                {
                  b: "hotfix/*",
                  c: "var(--rose)",
                  desc: "Critical prod fixes. From main, merges to main + develop.",
                },
              ].map((b) => (
                <div
                  key={b.b}
          ...
          <CodeBlock
            code={`# GitFlow CLI (optional tool)
git flow init
      ...
      {tab === "trunk" && (
        <div className="animate-fade-in">
          <Alert type="success" style={{ marginBottom: 20 }}>
            <strong>Trunk-Based Development</strong> is used by Google (Piper), Meta, Netflix,
          ...
          >
            ...
            <InfoCard icon={<Wrench size={18} />} title="Required Infrastructure" accentColor="var(--accent)">
              ...
              <br />
              • Deployment pipeline (canary deployments)
              <br />
              • Branch-by-abstraction for large changes
              <br />• Comprehensive observability
            </InfoCard>
          </div>
          <CodeBlock
            code={`# Trunk-based workflow
...
# Feature invisible until flag enabled for %users`}
      ...
      {tab === "commits" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 16 }}>
            Conventional Commits: <code>type(scope): description</code>. Enables automatic CHANGELOG
            generation and semantic versioning with semantic-release.
          </Alert>
            ...
            <tbody>
              {[
                ...
                [
                  ...
                  "patch",
                  "perf(query): add composite index on email",
                ],
                ...
                <tr key={type}>
                  <td>
                    <code style={{ color: "var(--cyan)" }}>{type}</code>
                  </td>
                  ...
                  <td style={{ fontSize: 11, fontFamily: "IBM Plex Mono", color: "var(--text)" }}>
                    {ex}
                  </td>
                </tr>
              ))}
...
# Reads commit types → bumps version → generates CHANGELOG → publishes`}
      ...
      {tab === "mergestrats" && (
        <div className="animate-fade-in">
          <table className="comp-table" style={{ marginBottom: 20 }}>
            ...
            <tbody>
              {[
                [
                  "Merge commit",
                  "Preserves all history with merge node",
                  "1 merge commit (+ all originals)",
                  "GitFlow, feature branches needing context",
                ],
                [
                  "Squash merge",
                  "Linear — one commit per PR",
                  "1 commit (all changes squashed)",
                  "Clean main, trunk-based dev, code review",
                ],
                [
                  "Rebase merge",
                  "Linear — all commits inlined with new SHAs",
                  "n commits (new SHAs)",
                  "Linear + full granularity, small PRs",
                ],
                [
                  "Fast-forward",
                  "Linear — no merge commit at all",
                  "0 extra (pointer moves)",
                  "Small branches, no divergence",
                ],
              ].map(([strat, hist, commits, use]) => (
                <tr key={strat}>
                  <td style={{ fontWeight: 600 }}>{strat}</td>
                  <td style={{ fontSize: 12, color: "var(--muted)" }}>{hist}</td>
                  <td style={{ fontSize: 12, fontFamily: "IBM Plex Mono", color: "var(--cyan)" }}>
                    {commits}
                  </td>
          ...
          <Alert type="warn">
            <strong>Pick ONE and enforce it.</strong> Mixing strategies in a repo makes git log and
            git bisect unreliable. Configure in GitHub/GitLab branch protection rules.
          </Alert>
        </div>
      )}
      ...
      {tab === "monorepo" && (
        <div className="animate-fade-in">
          <Alert type="info" style={{ marginBottom: 16 }}>
            Monorepos (single repo, multiple packages/apps) need specialized Git tooling. Key
          ...
          >
            <div>
              ...
              <CodeBlock
                code={`# Only checkout needed directories
...
git sparse-checkout list`}
            ...
            <div>
              ...
              <CodeBlock
                code={`# Treeless: no file blobs upfront
...
# Reduces clone time dramatically`}
          ...
          <CodeBlock
            code={`# CODEOWNERS for monorepo ownership
# .github/CODEOWNERS
      ...
      {tab === "forcepush" && (
        <div className="animate-fade-in">
          <Alert type="danger" style={{ marginBottom: 20 }}>
            <strong>git push --force</strong> overwrites remote history WITHOUT checking if anyone
            has pushed since your last fetch. This can destroy teammates' work.
    ...
    <div className="animate-fade-up">
      ...
      <Alert type="info" style={{ marginBottom: 28 }}>
        <span>
          ...
          <code>git reflog</code>. Your commits survive for 90 days after becoming unreachable. The
          one true exception: <strong>never commit credentials</strong> — rotate immediately if you
          do.
        </span>
      </Alert>
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 240 }}>
                ...
                >
                  <h3 className="font-heading" style={{ fontWeight: 700, fontSize: 16 }}>
                    {s.title}
                  </h3>
                  <span
                    style={{
  ...
  const comparisons = {
    "merge-vs-rebase": {
      ...
      rows: [
        ...
        [
          "Traceability",
          "Clear when branches diverged and merged",
          "Harder to trace original feature branch",
        ],
        ...
        [
          "Use case",
          "GitFlow, preserving full context, shared branches",
          "Feature cleanup before PR, local branches",
        ],
    ...
    "reset-vs-revert": {
      ...
      rows: [
        [
          "History",
          "⚠️ Rewrites history — commit disappears",
          "✅ Creates new commit — history preserved",
        ],
    ...
    "switch-vs-checkout": {
      ...
      rows: [
        ...
        [
          "Purpose",
          "Branch switching ONLY — single responsibility",
          "Both branches AND file restoration (overloaded)",
        ],
    ...
    "fetch-vs-pull": {
      ...
      rows: [
        [
          "What it does",
          "Downloads remote changes only (no integration)",
          "Downloads + integrates (fetch + merge/rebase)",
        ],
        ["Working tree", "✅ Never modified", "Updated to merged/rebased state"],
        [
          "Safe to run anytime",
          "✅ Always 100% safe",
          "May cause conflicts or unwanted merge commits",
        ],
        ["Control", "Maximum — review before integrating", "Convenient but less control"],
        [
          "Typical use",
          "See what changed → decide how to integrate",
          "Quick sync when working tree is clean",
        ],
        [
          "Best practice",
          "git fetch → git log origin/main → git rebase",
          "git pull --rebase (avoids merge commits)",
        ],
        [
          "Team awareness",
          "Lets you see others' work before integration",
          "Integrates immediately without review",
        ],
      ],
    },
    "github-vs-gitlab": {
      ...
      rows: [
        ...
        [
          "CI/CD",
          "GitHub Actions (excellent, marketplace ecosystem)",
          "GitLab CI/CD (built-in, very mature)",
        ],
        ...
        [
          ...
          "Built-in per-project registry",
        ],
        [
          "Security scanning",
          "Via Actions marketplace + partners",
          "Built-in SAST/DAST/dependency scanning",
        ],
        ["Package registry", "GitHub Packages", "Built-in package registry (all formats)"],
        [
          "Best for",
          "OSS community, developer tools, Microsoft ecosystem",
          "Enterprise, regulated industries, self-host needs",
        ],
      ],
    },
  };
            ...
            <table className="comp-table">
              ...
              <tbody>
                {comp.rows.map(([dim, l, r], i) => (
                  <tr key={i}>
                    <td style={{ fontWeight: 600, whiteSpace: "nowrap", color: "var(--muted)" }}>
                      {dim}
                    </td>
